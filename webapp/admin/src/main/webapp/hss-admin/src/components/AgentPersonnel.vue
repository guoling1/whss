      ...
      <div class="box" style="margin-top:15px;overflow: hidden">
        ...
        <div class="box-body">
          ...
          <el-table v-loading.body="loading" style="font-size: 12px;margin:15px 0" :data="records" border>
            ...
            <el-table-column prop="belongDealer" label="所属代理商"></el-table-column>
            ...
            <el-table-column label="操作" width="100">
              <template scope="scope">
                <a @click="_$power(scope.row.id,issue1,'boss_dealer_staff_update')" type="text" size="small">编辑</a>
                <a @click="_$power(scope.row.id,open,'boss_dealer_staff_disable')" v-if="records[scope.$index].status==2" type="text" size="small">开启</a>
...
<script lang="babel">
  export default{
    ...
    methods: {
      ...
      open: function (val) {
        this.loading = true;
        this.$http.post('/admin/dealer/activeUser',{id:val})
          .then((res)=>{
      ...
      close: function (val) {
        this.loading = true;
        this.$http.post('/admin/dealer/disableUser',{id:val})
          .then((res)=>{
            for(var i=0;i<this.records.length;i++){
              if(this.records[i].id == val){
