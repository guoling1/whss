          ...
          <el-tabs class="tab" v-model="activeName" type="card" @tab-click="handleClick">
            <el-tab-pane label="好收收" name="first">
              ...
              <el-table v-loading.body="loading" style="font-size: 12px;margin:15px 0" :data="records" border>
                ...
                <el-table-column prop="source" label="注册方式"></el-table-column>
            ...
            <el-tab-pane label="好收银" name="second">
              ...
              <el-table v-loading.body="loading" style="font-size: 12px;margin:15px 0" :data="records" border>
                ...
                <el-table-column prop="createTime" label="注册时间"></el-table-column>
                <el-table-column prop="contactCellphone" label="注册手机号"></el-table-column>
                <el-table-column prop="districtCode" label="省市"></el-table-column>
                <el-table-column prop="industryCode" label="行业"></el-table-column>
                <el-table-column prop="status" label="状态"></el-table-column>
                <el-table-column label="操作" width="100">
                  <template scope="scope">
                    <router-link :to="{path:'/admin/record/StoreAuditHSY',query:{id:records[scope.$index].id,status:records[scope.$index].status}}" v-if="records[scope.$index].status==2" type="text" size="small">审核</router-link>
                    <router-link :to="{path:'/admin/record/StoreAuditHSY',query:{id:records[scope.$index].id,status:records[scope.$index].status}}" v-if="records[scope.$index].status!=2" type="text" size="small">查看详情</router-link>
                  </template>
                </el-table-column>
              </el-table>
            </el-tab-pane>
