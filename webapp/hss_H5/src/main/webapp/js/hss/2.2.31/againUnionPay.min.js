...
var uorderId = getQueryString('orderId');
...
submit.addEventListener('click', function () {
  if (pageData.canPay) {
    if ((pageData.showExpireDate == 0 || validate.empty(expireDate.innerHTML, '信用卡有效期')) && (pageData.showCvv == 0 || validate.empty(cvv2.value, 'CVV2')) && validate.empty(code.value, '验证码')) {
...
sendCode.addEventListener('click', function () {
  if (pageData.canPay) {
    if (countdown.check()) {
      if ((pageData.showExpireDate == 0 || validate.empty(expireDate.innerHTML, '信用卡有效期')) && (pageData.showCvv == 0 || validate.empty(cvv2.value, 'CVV2'))) {
        if (pageData.showCvv == 0 || cvv2.value.length == 3) {
