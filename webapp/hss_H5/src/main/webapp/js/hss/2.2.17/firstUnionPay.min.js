...
submit.addEventListener('click', function () {
  if (!support) {
    message.prompt_show('请输入正确的信用卡号');
  } else if ((pageData.showExpireDate == 0 || validate.empty(expireDate.value, '信用卡有效期')) && (pageData.showCvv == 0 || validate.empty(cvv2.value, 'CVV2')) && validate.phone(mobile.value) && validate.empty(code.value, '验证码')) {
...
sendCode.addEventListener('click', function () {
  if (countdown.check()) {
    if (!support) {
      message.prompt_show('请输入正确的信用卡号');
    } else if ((pageData.showExpireDate == 0 || validate.empty(expireDate.value, '信用卡有效期')) && (pageData.showCvv == 0 || validate.empty(cvv2.value, 'CVV2')) && validate.phone(mobile.value)) {
      if (pageData.showCvv == 0 || cvv2.value.length == 3) {
        message.load_show('正在发送');
        var expire = expireDate.value.split('/');
