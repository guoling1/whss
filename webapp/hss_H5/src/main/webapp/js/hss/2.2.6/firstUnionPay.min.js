...
submit.addEventListener('click', function () {
  ...
  } else if (validate.empty(expireDate.value, '信用卡有效期') && validate.empty(cvv2.value, 'CVV2') && validate.phone(mobile.value) && validate.empty(code.value, '验证码')) {
...
sendCode.addEventListener('click', function () {
  if (countdown.check()) {
    if (!support) {
      message.prompt_show('请输入正确的信用卡号');
    } else if (validate.empty(expireDate.value, '信用卡有效期') && validate.empty(cvv2.value, 'CVV2') && validate.phone(mobile.value)) {
