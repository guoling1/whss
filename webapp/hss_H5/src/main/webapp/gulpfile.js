...
gulp.task('less-hss', function () {
  return gulp.src('less/**/hss.less')
    ...
    .pipe(rename({basename: "style.2.2.10"}))
    .pipe(gulp.dest('css'));
});

gulp.task('replace-hss-calc', ['build-hss'], function () {
  return gulp.src('css/hss/style.2.2.10.css')
...
gulp.task('js-hss', () => {
  return gulp.src('es6/hss/*.js')
    ...
    .pipe(rename({suffix: ".min"}))
    .pipe(sourcemaps.write('.'))
    .pipe(gulp.dest('js/hss/2.2.16'));
});

gulp.task('replace-hss', function () {
  return gulp.src('WEB-INF/jsp/*.jsp')
    .pipe(replace('vendor.1.0.9.7.min.js', 'vendor.1.0.9.8.min.js'))
    .pipe(replace('style.2.2.9.css', 'style.2.2.10.css'))
    .pipe(replace('style.2.0.2.css', 'style.2.0.3.css'))
    .pipe(replace('/hss/2.2.15/', '/hss/2.2.16/'))
