...
gulp.task('less-hss', function () {
  return gulp.src('less/**/hss.less')
    ...
    .pipe(rename({basename: "style.2.1.8"}))
    .pipe(gulp.dest('css'));
});

gulp.task('replace-hss-calc', ['build-hss'], function () {
  return gulp.src('css/hss/style.2.1.8.css')
...
gulp.task('js-hss', () => {
  return gulp.src('es6/hss/*.js')
    ...
    .pipe(rename({suffix: ".min"}))
    .pipe(sourcemaps.write('.'))
    .pipe(gulp.dest('js/hss/2.1.6'));
});

gulp.task('replace-hss', function () {
  return gulp.src('WEB-INF/jsp/*.jsp')
    .pipe(replace('style.2.1.5.css', 'style.2.1.8.css'))
    .pipe(replace('vendor.1.0.3.min.js', 'vendor.1.0.6.min.js'))
