...
gulp.task('less-hss', function () {
  return gulp.src('less/**/hss.less')
    ...
    .pipe(rename({basename: "style.2.2.21"}))
    .pipe(gulp.dest('css'));
});

gulp.task('replace-hss-calc', ['build-hss'], function () {
  return gulp.src('css/hss/style.2.2.21.css')
...
gulp.task('less-hss-beta', function () {
  return gulp.src('less/**/hss.2.0.2.less')
    ...
    .pipe(rename({basename: "style.2.0.5"}))
    .pipe(gulp.dest('css'));
});
...
gulp.task('js-hss', () => {
  return gulp.src('es6/hss/*.js')
    ...
    .pipe(rename({suffix: ".min"}))
    .pipe(sourcemaps.write('.'))
    .pipe(gulp.dest('js/hss/2.2.30'));
});

gulp.task('replace-hss', function () {
  return gulp.src('WEB-INF/jsp/*.jsp')
    .pipe(replace('vendor.1.0.9.11.min.js', 'vendor.1.0.9.12.min.js'))
    .pipe(replace('style.2.2.20.css', 'style.2.2.21.css'))
    .pipe(replace('style.2.0.4.css', 'style.2.0.5.css'))
    .pipe(replace('/hss/2.2.29/', '/hss/2.2.30/'))
