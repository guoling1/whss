...
public class MerchantWithdrawServiceImpl implements MerchantWithdrawService {
    ...
    public Pair<Integer, String> withdraw(long accountId, String totalAmount, int channel, String appId, BigDecimal withDrawFee) {
        ...
        Preconditions.checkArgument(merchantInfoOptional.isPresent(), "商户不存在");
        final MerchantInfo merchantInfo = merchantInfoOptional.get();
        final long playMoneyOrderId = this.orderService.createMerchantPlayMoneyOrder(merchantInfo, new BigDecimal(totalAmount),
